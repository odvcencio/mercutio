...
func main() {
	...
	operatorMutation := func(handler http.Handler) http.Handler {
		return authn.Require(authn.ProtectBrowser(handler))
	}
	...
	app.Mount("POST /api/cells", operatorMutation(http.HandlerFunc(apiHandler.CreateCell)))
	...
	app.Mount("POST /api/cells/{cellID}/events", operatorMutation(http.HandlerFunc(apiHandler.RecordEvent)))
	app.Mount("POST /api/cells/{cellID}/attach-token", operatorMutation(http.HandlerFunc(apiHandler.AttachToken)))
	app.Mount("GET /api/cells/{cellID}/capability", authn.Require(http.HandlerFunc(apiHandler.OperatorCapability)))
	app.Mount("POST /api/cells/{cellID}/secrets", operatorMutation(http.HandlerFunc(apiHandler.PutSecret)))
	app.Mount("GET /api/cells/{cellID}/secrets", authn.Require(http.HandlerFunc(apiHandler.SecretDescriptors)))
	app.Mount("POST /api/cells/{cellID}/secret-capability", operatorMutation(http.HandlerFunc(apiHandler.SecretCapability)))
	...
	app.Mount("POST /api/cells/{cellID}/secret-proxies", operatorMutation(http.HandlerFunc(apiHandler.ConfigureSecretProxy)))
	...
	app.Mount("POST /api/cells/{cellID}/analyze", operatorMutation(http.HandlerFunc(apiHandler.Analyze)))
	app.Mount("POST /api/cells/{cellID}/edit", operatorMutation(http.HandlerFunc(apiHandler.Edit)))
	app.Mount("POST /api/cells/{cellID}/undo", operatorMutation(http.HandlerFunc(apiHandler.UndoEdit)))
	app.Mount("POST /api/cells/{cellID}/files/delete", operatorMutation(http.HandlerFunc(apiHandler.DeleteFile)))
	...
	app.Mount("POST /api/cells/{cellID}/reviews/{reviewID}/reject", operatorMutation(http.HandlerFunc(apiHandler.RejectReview)))
	app.Mount("POST /api/cells/{cellID}/shadows/{shadowID}/{operation}", operatorMutation(http.HandlerFunc(apiHandler.ShadowDecision)))
