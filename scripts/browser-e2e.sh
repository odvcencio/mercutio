...
MERCUTIO_PORT="$port" MERCUTIO_DEV_MODE=1 MERCUTIO_STATE_PATH="$tmp/state.json" MERCUTIO_EVIDENCE_PATH="$tmp/evidence.jsonl" ./bin/mercutio >"$tmp/server.log" 2>&1 &
server_pid=$!
...
MERCUTIO_E2E_URL="http://127.0.0.1:$port/?cell=$cell_id&file=cmd%2Fhello%2Fmain.go" \
  ...
  "${GO:-go}" test ./internal/e2e -run TestGoSXEditorIntelligence -count=1
